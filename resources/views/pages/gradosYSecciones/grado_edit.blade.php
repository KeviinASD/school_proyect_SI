...
@section('content')
    <section class="space-y-4">
        ...
        <form action="{{ route('grados.update', $grado->idGrado) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div class="space-y-2">
                    <label for="nombreGrado" class="block text-gray-700">Nombre del Grado</label>
                    <input class="focus:outline-none border border-gray-300 p-2 rounded-sm w-full" type="text" name="nombreGrado" id="nombreGrado" placeholder="Nombre del Grado" required value="{{$grado->nombreGrado}}">
                    @error('nombreGrado')
                ...
                <div class="space-y-2">
                    ...
                    <select name="idNivel" id="idNivel" class="focus:outline-none border border-gray-300 p-2 rounded-sm w-full" required >
                        @foreach ($niveles as $nivel)
                            <option value="{{ $nivel->idNivel }}" {{ $grado->idNivel == $nivel->idNivel ? 'selected' : '' }}>{{ $nivel->nombreNivel }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex justify-between">
                    <button class="px-4 py-2 rounded bg-[#DEF4DB] font-semibold hover:bg-green-300 transition duration-300" type="submit">Editar Grado</button>
